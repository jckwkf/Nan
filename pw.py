...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if datetime.now(UTC) - granted_time > timedelta(hours=1):
...
# Flask dummy server to keep port open
...
def run_flask():
    app_flask.run(host='0.0.0.0', port=10000)  # Port number can be changed if needed
...
if __name__ == "__main__":
    try:
        ...
        # Start Flask server in a separate thread to keep port open
